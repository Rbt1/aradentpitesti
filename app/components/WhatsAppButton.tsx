'use client'

import { useState } from 'react'
import { motion } from 'framer-motion'
import { trackEvent } from '@/lib/analytics'
import QuizModal from './QuizModal'

const WHATSAPP_URL = 'https://wa.me/40754219011'
const PHONE_URL = 'tel:+40754219011'

const PhoneIcon = () => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
    xmlns="http://www.w3.org/2000/svg"
    className="w-6 h-6 text-cream"
    aria-hidden="true"
  >
    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
  </svg>
)

const ToothIcon = () => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="white"
    strokeWidth="1.8"
    strokeLinecap="round"
    strokeLinejoin="round"
    width="16"
    height="16"
    aria-hidden="true"
  >
    <path d="M12 2C8 2 5 5.5 5 9.5c0 2.5 1 5 2 7.5L8.5 22h2l1.5-5 1.5 5h2L17 17c1-2.5 2-5 2-7.5C19 5.5 16 2 12 2z" />
  </svg>
)

const WhatsAppButton = () => {
  const [quizOpen, setQuizOpen] = useState(false)

  return (
    <>
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">

        {/* Buton Quiz Dentar */}
        <div style={{ position: 'relative' }}>
          {/* Inel puls */}
          <motion.span
            style={{
              position: 'absolute',
              inset: '-4px',
              borderRadius: '54px',
              backgroundColor: '#C9A84C',
              opacity: 0,
              pointerEvents: 'none',
            }}
            animate={{ opacity: [0, 0.45, 0], scale: [0.95, 1.12, 1.12] }}
            transition={{ duration: 0.9, repeat: Infinity, repeatDelay: 2.1, ease: 'easeOut' }}
          />
          <motion.button
            onClick={() => setQuizOpen(true)}
            aria-label="Deschide quiz sănătate dentară"
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ delay: 1.4, duration: 0.4, ease: [0.25, 0.46, 0.45, 0.94] }}
            whileHover={{ scale: 1.05, opacity: 0.9 }}
            whileTap={{ scale: 0.96 }}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
              padding: '12px 20px',
              backgroundColor: '#C9A84C',
              color: '#1B4332',
              border: 'none',
              borderRadius: '50px',
              cursor: 'pointer',
              fontFamily: 'var(--font-jost)',
              fontSize: '13px',
              fontWeight: 700,
              letterSpacing: '0.02em',
              boxShadow: '0 4px 16px rgba(201,168,76,0.35)',
              whiteSpace: 'nowrap',
              position: 'relative',
            }}
          >
            <ToothIcon />
            Quiz Dentar 🎁
          </motion.button>
        </div>

        {/* Buton telefon */}
        <motion.a
          href={PHONE_URL}
          aria-label="Sună la ARA DENT STUDIO"
          className="w-14 h-14 rounded-full bg-forest flex items-center justify-center shadow-lg hover:shadow-xl relative"
          data-ga-manual="true"
          onClick={() => trackEvent('phone_click', { location: 'floating_button' })}
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ delay: 1.7, duration: 0.4, ease: [0.25, 0.46, 0.45, 0.94] }}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        >
          <motion.span
            className="absolute inset-0 rounded-full bg-forest"
            animate={{ scale: [1, 1.4, 1], opacity: [0.5, 0, 0.5] }}
            transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
          />
          <PhoneIcon />
        </motion.a>

        {/* Buton WhatsApp */}
        <motion.a
          href={WHATSAPP_URL}
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Contactează ARA DENT STUDIO pe WhatsApp"
          className="w-14 h-14 rounded-full bg-[#25D366] flex items-center justify-center shadow-lg hover:shadow-xl"
          data-ga-manual="true"
          onClick={() => trackEvent('whatsapp_click', { location: 'floating_button' })}
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ delay: 2, duration: 0.4, ease: [0.25, 0.46, 0.45, 0.94] }}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        >
          <svg
            viewBox="0 0 24 24"
            fill="white"
            xmlns="http://www.w3.org/2000/svg"
            className="w-7 h-7"
            aria-hidden="true"
          >
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
        </motion.a>
      </div>

      <QuizModal isOpen={quizOpen} onClose={() => setQuizOpen(false)} />
    </>
  )
}

export default WhatsAppButton
